...
|---|---|---|
| Backend (Java / Spring Boot) | [nachbau-yfinance-backend](https://github.com/JTrachsler2008/nachbau-yfinance-backend) | fertig, 34 Tickets, 121 Tests grün |
...
- **MUI 9** als Komponentenbibliothek, Light und Dark über CSS-Variablen
- **React Query** für Server-State, **axios** für den HTTP-Zugriff
- **Vitest** und **Testing Library** für Tests
- Responsive Umsetzung: Tabellen werden auf kleinen Viewports zu Karten

Der UI/UX-Plan nennt MUI v6. Umgesetzt ist v9, Begründung in
...
Grundgerüst und Auth-Flow über die Fach-Seiten (Portfolios, Konten, Transaktionen,
Dashboard, Performance, Risiko, Vergleiche, Simulationen) bis zu Fehlerbehandlung,
Responsive-Umsetzung und den rollenabhängigen Bereichen für Manager und Admin.
Vollständige Liste in `nachbau/planung/jira-tickets-backlog.md`.
...
Das Backend muss dafür lokal laufen (`localhost:8080`, siehe Backend-Repo). Der Dev-Server leitet
`/api` dorthin weiter, weil das Backend keine CORS-Konfiguration hat und ein direkter Aufruf sonst
vom Browser blockiert würde.
...
Zu den Tests: die Farbtokens des Themes werden gegen die WCAG-Kontrastschwellen geprüft
(`src/theme/palette.test.ts`). Wer eine Farbe im Theme ändert, muss damit rechnen, dass diese Tests
