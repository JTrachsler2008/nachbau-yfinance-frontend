...
/** Farbwerte eines Transaktions-Badges (Fläche plus Schrift). */
 ...
 * und die im Original verbreitete Inline-`style={{}}`-Praxis kehrt nicht zurück.
...
export interface FinancePalette {
   ...
   * damit WCAG AA für normalen Text verfehlt. Der gewählte Ton hält AA auf allen drei
   * Light-Flächen: 7.13:1 auf der Karte, 6.68:1 auf der abgesetzten Fläche, 6.30:1 auf dem
   * Seitengrund. Der nächsthellere Ton `#15803d` wäre auf dem Seitengrund mit 4.43:1 durchgefallen.
   */
  gainText: string
  /**
   ...
   */
  lossText: string
  /** Abgesetzte Fläche für Tabellenkopf und Hover-Zeile. */
  surfaceMuted: string
  badgeBuy: FinanceBadge
  badgeSell: FinanceBadge
  badgeDividend: FinanceBadge
  /**
   * Kategoriale Diagrammfarben für Recharts. Recharts kennt das MUI-Theme nicht, deshalb müssen
   * Diagrammfarben eigens als Token vorliegen, sonst bleiben sie beim Moduswechsel hängen.
   * Beginnt bewusst nicht mit Grün oder Rot, damit eine kategoriale Einfärbung (Sektor, Land,
   * Währung) nicht mit der Gewinn-/Verlust-Semantik verwechselt wird.
   */
  chart: readonly string[]
}

/**
 * Light: die Palette des Originals (`yfinance-frontend/src/index.css`), unverändert übernommen
...
export const lightFinance: FinancePalette = {
  ...
  lossText: '#b91c1c',
  surfaceMuted: '#faf7f4',
  // Badge-Schrift nutzt denselben Ton wie die Zahlen, damit es nicht zwei Grün- und zwei Rottöne
  // für dieselbe Bedeutung gibt.
...
/**
 * Dark: warm gehalten statt neutral-blaugrau. Ein MUI-Default-Dunkel (`#121212`) würde neben der
...
export const darkFinance: FinancePalette = {
  ...
  lossText: '#f87171',
  surfaceMuted: '#2b2521',
  // Feste Pastelltöne wie #dcfce7 wirken auf dunklem Grund grell, daher die Semantikfarbe
  // transparent als Fläche.
