...
/**
 * Getestet wird ueber einen ausgetauschten axios-Adapter, nicht ueber eine nachgebaute Kopie der
 * Umsetzungslogik. So laeuft die echte Interceptor-Kette des exportierten `apiClient` durch, und
 * ein Fehler in der Verdrahtung der Interceptoren faellt auf.
 */
...
/** Antwortet erfolgreich und merkt sich die Anfrage, damit Header geprueft werden koennen. */
...
describe('Konfiguration', () => {
  it('nutzt ohne VITE_API_BASE_URL den relativen Pfad /api', () => {
    // Der relative Pfad ist Voraussetzung dafuer, dass der Dev-Proxy greift und die fehlende
    // CORS-Konfiguration des Backends nicht auffaellt.
    expect(apiBaseUrl).toBe('/api')
    expect(apiClient.defaults.baseURL).toBe('/api')
  })

  it('setzt einen Timeout, damit ein haengendes Backend die UI nicht blockiert', () => {
    expect(apiClient.defaults.timeout).toBe(30_000)
  })
})

describe('Auth-Token', () => {
  it('haelt den Token nur im Speicher und nicht in localStorage', () => {
    setAuthToken('geheim-123')
    expect(getAuthToken()).toBe('geheim-123')
    // SEC-1/SEC-2 des Architektur-Plans: ein Token in der Storage waere per XSS auslesbar.
    // Geprueft wird, dass ueberhaupt nichts geschrieben wurde, nicht nur der Schluessel.
  ...
  it('haengt den gesetzten Token als Bearer an', async () => {
...
describe('Fehler-Uebersetzung', () => {
  it('uebernimmt Message und fieldErrors aus der Backend-Struktur', async () => {
    const body: BackendErrorBody = {
      timestamp: '2026-08-24T12:00:00Z',
      status: 400,
      error: 'Bad Request',
      message: 'Nicht genuegend Cash auf dem Konto',
      fieldErrors: { quantity: 'muss groesser als 0 sein' },
    }
    ...
    expect(error.message).toBe('Nicht genuegend Cash auf dem Konto')
    expect(error.status).toBe(400)
    expect(error.fieldErrors).toEqual({ quantity: 'muss groesser als 0 sein' })
  ...
  it('faellt auf die axios-Message zurueck, wenn die Antwort nicht der Backend-Struktur entspricht', async () => {
  ...
  it('uebersetzt auch Fehler, die nicht von axios kommen', async () => {
  ...
  it('gibt eine erfolgreiche Antwort unveraendert durch', async () => {
    installAdapter((config) =>
      Promise.resolve({
